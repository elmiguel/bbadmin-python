...
Usage:
    student-replace-batch-uid.py OLD-ID NEW-ID USER-ID FIRSTNAME LASTNAME [PASSWD] EMAIL [options]
    student-replace-batch-uid.py --lookup LOOKUP-TYPE LOOKUP-VALUE [options]
    student-replace-batch-uid.py (-h | --help)
    student-replace-batch-uid.py (-v | --version)
    ...
    if args['--lookup']:
        ...
        with open(filename, 'w') as o:
            headers = ['external_person_key', 'user_id', 'firstname', 'lastname',
                       'email', 'new_external_person_key', 'passwd' if passwd not '' else '', 'data_source_key']
